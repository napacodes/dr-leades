                    ...
                    <h4 class="card-title">{{ __('content.edit_service') }}
                        <div class="btn-group">
                            ...
                            <div class="dropdown-menu">
                                ...
                                <a class="dropdown-item"  href="{{ route('service-info.create', $service->id) }}">{{ __('content.service_info') }}</a>
                                <a class="dropdown-item"  href="{{ route('service-feature.create', $service->id) }}">{{ __('content.service_features') }}</a>
                            </div>
                        </div>
                        <!-- Button -->
                ...
                @else
                    <form action="{{ route('service.update', $service->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-12">
                                    <div class="form-group">
                                        <label for="section_image_2">{{ __('content.thumbnail') }} ({{ __('content.size') }} 600 x 600) (.svg, .jpg, .jpeg, .png, .webp, .gif)</label>
                                    ...
                                    <div class="height-card box-margin">
                                        <div class="card">
                                            <div class="card-body">
                                                <div class="avatar-area text-center">
                                                    <div class="media">
                                                        @if (!empty($service->section_image_2))
                                                            <a  class="d-block mx-auto" href="#" data-toggle="tooltip" data-placement="top" data-original-title="{{ __('content.current_image') }}">
                                                                <img src="{{ asset('uploads/img/service/'.$service->section_image_2) }}" alt="image" class="rounded w-25">
                                                            </a>
                                                        @else
                                                            <a class="d-block mx-auto" href="#" data-toggle="tooltip" data-placement="top" data-original-title="{{ __('content.not_yet_created') }}">
                                                                <img src="{{ asset('uploads/img/dummy/no-image.jpg') }}" alt="no image" class="rounded w-25">
                                                    ...
                                                    @if (!empty($service->section_image_2))
                                                        <a class="mt-3 d-block" href="#" data-toggle="modal" data-target="#deleteImageModal2{{ $service->id }}">
                                                            <i class="fa fa-trash text-danger font-18"></i>
                                                        </a>
                                                    @endif
                                                </div>
                    ...
                    <!-- Modal -->
                    <div class="modal fade" id="deleteImageModal2{{ $service->id }}" tabindex="-1" role="dialog" aria-labelledby="deleteImageModal2CenterTitle" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered modal-sm" role="document">
                            <div class="modal-content">
                                <div class="modal-header">
                                ...
                                <div class="modal-footer">
                                    <form class="d-inline-block" action="{{ route('service.destroy_image_2', $service->id) }}" method="POST">
                                        @method('DELETE')
                                        @csrf
                                        <button type="button" class="btn btn-danger mr-1" data-dismiss="modal">{{ __('content.cancel') }}</button>
                                        <button type="submit" class="btn btn-success">{{ __('content.yes_delete_it') }}</button>
