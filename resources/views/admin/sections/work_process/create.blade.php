                            ...
                            <table id="basic-datatable" class="table table-striped dt-responsive w-100">
                                ...
                                @foreach ($items as $item)
                                    <tr>
                                        ...
                                        <td>
                                            <div>
                                                <a href="{{ route('work-process.edit', $item->id) }}" class="mr-2">
                                            ...
                                            <div class="modal-content">
                                                ...
                                                @if ($demo_mode == "on")
                                                    <!-- Include Alert Blade -->
                                                        @include('admin.demo_mode.demo-mode')
                                                    @else
                                                        <form class="d-inline-block" action="{{ route('work-process.destroy', $item->id) }}" method="POST">
                                                            @method('DELETE')
                                                            @csrf
                                                            @endif
                ...
                <div class="modal-body">
                    @if (isset($item_section))
                        @if ($demo_mode == "on")
                            ...
                            @include('admin.demo_mode.demo-mode')
                        @else
                            <form action="{{ route('work-process-section.update', $item_section->id) }}" method="POST">
                                @method('PUT')
                                @csrf
                                @endif
                    ...
                    @else
                        <form action="{{ route('work-process.store') }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="form-group" >
                                        <label for="section_image">{{ __('content.image') }} ({{ __('content.size') }} 328 x 328 (.svg, .jpg, .jpeg, .png, .webp, .gif)</label>
